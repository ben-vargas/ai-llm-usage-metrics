...
- `--provider` matches canonical billing entities after provider normalization.
- Provider aliases are normalized to billing roots first (for example, `openai-codex` is treated as `openai`).
