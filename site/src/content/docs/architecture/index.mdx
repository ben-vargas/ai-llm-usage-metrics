...
`optimize` reporting reuses usage parsing/filtering and evaluates candidate-model counterfactual pricing on observed token totals.

Provider fields are normalized to billing-entity identifiers at event normalization time (for example, `openai-codex` normalizes to `openai`).
...
12. `optimize` path: `buildUsageData` + counterfactual engine → `renderOptimizeReport`
...
- `src/optimize`: candidate-model counterfactual computation + optimize row contracts
