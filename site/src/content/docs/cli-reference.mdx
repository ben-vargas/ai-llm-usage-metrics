...
Generated from CLI version `0.3.2` via `scripts/generate-cli-reference.mjs`.
...
| `--include-merge-commits` | - | - | Include merge commits in Git outcome metrics |
...
| `--repo-dir` | - | `<path>` | Path to repository for Git outcome metrics |
...
`efficiency` commands attribute usage events to Git repository roots based on source metadata (`cwd`/path fields). Only events attributed to the selected repository contribute to efficiency totals.
