...
- If pricing fails in offline mode, run once without `--pricing-offline` to warm the cache.
- If reports look stale after source file changes, verify source files updated `mtime` and cache TTL settings.
- If you need fresh update checks every run, set `LLM_USAGE_UPDATE_CACHE_TTL_MS=0`.
